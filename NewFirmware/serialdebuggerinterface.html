// =============================================================================
// Dr. Water Smart Counter - WEB APPLICATION UI
// By Lian Mollick
// Version: 3.6 (Final for ESP8266 Standalone Server)
// This entire file should be placed in a tab named "index.h"
// in the Arduino IDE.
// =============================================================================
...
<head>
    ...
    <style>
        ...
        .modal-buttons .cancel-btn { background-color: #e5e5ea;}
...
<body>
    ...
    <div class="main-container">
        <header class="header">
            <h1>
                ...
                <span id="version-tag">v3.6 - Standalone</span>
            </h1>
    ...
    <div id="modal" class="modal-overlay">
        <div class="modal-content">
            <h2 id="modal-title">Admin Authentication</h2>
            <div id="limit-input-group" class="form-group" style="display: none;">
                <label for="new-limit-value">New Limit (Liters)</label>
                <input type="number" id="new-limit-value">
            </div>
            ...
            <div class="modal-buttons">
                ...
                <button id="submit-login" class="submit-btn">Submit</button>
            </div>
        </div>
    </div>
    ...
    <script>
        ...
        let pendingAction = { type: null, cartridge: null };
                ...
                container.innerHTML += `
                    <div class="cartridge-item" id="cartridge-${i}">
                        ...
                        <div class="cartridge-controls">
                            ...
                            <button onclick="showLogin('setLimit', ${i})">Change Limit</button>
                        </div>
                    </div>
                `;
            }
        ...
        async function postCommand(endpoint, formData) {
             try {
        ...
        function showLogin(actionType, cartridgeIndex = null) {
            stopDataFetching();
            if (actionType === 'hardReset' && !confirm('Are you sure? This will erase all data.')) {
                startDataFetching(); return;
            }
            pendingAction.type = actionType;
            pendingAction.cartridge = cartridgeIndex;
            
            const loginTitle = document.getElementById('modal-title');
            const limitInputGroup = document.getElementById('limit-input-group');

            if (actionType === 'setLimit') {
                loginTitle.textContent = `Set Limit for Cartridge #${cartridgeIndex}`;
                const currentLimit = document.getElementById(`cartridge-${cartridgeIndex}-limit`).textContent.replace(' L', '');
                document.getElementById('new-limit-value').value = currentLimit;
                limitInputGroup.style.display = 'block';
            } else {
                loginTitle.textContent = 'Admin Authentication';
                limitInputGroup.style.display = 'none';
            }
        ...
        document.getElementById('submit-login').onclick = async function() {
            ...
            if (pendingAction.type === 'hardReset') {
                endpoint = '/command';
                formData.append('cmd', 'h');
            } else if (pendingAction.type === 'reset') {
                endpoint = '/command';
                formData.append('cmd', `c=${pendingAction.cartridge}`);
            } else if (pendingAction.type === 'setLimit') {
                endpoint = '/setlimits';
                // When setting limits, the firmware expects all 3
                for(let i=1; i<=3; i++) {
                    let limitVal;
                    if (i === pendingAction.cartridge) {
                        limitVal = document.getElementById('new-limit-value').value;
                        if (!limitVal || parseInt(limitVal) <= 0) {
                         ...
                         limitVal = document.getElementById(`cartridge-${i}-limit`).textContent.replace(' L', '');
                    }
                     formData.append(`limit${i}`, limitVal);
                }
            }
            ...
            if (endpoint) {
                await postCommand(endpoint, formData);
            }
        };
